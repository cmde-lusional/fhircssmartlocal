...
namespace fhircssmartlocal
{
    ...
    public class Program
    {
        ...
        public static string fhirServerUrl = _fhirServers["Smart"];
        ...
        static int Main(string[] args)
        {
            ...
            Task.Factory.StartNew(() => { CreateHostBuilder().Build().Run(); });
        ...
        public static async void setAuthCode(string code, string state)
        {
            ...
            Console.WriteLine("----- Authorzation Response ------");
            Console.WriteLine(json);
            Console.WriteLine("----- Authorzation Response ------");

            //Deserialize the http json response
            SmartResponse smartResponse = JsonSerializer.Deserialize<fhircssmartlocal.SmartResponse>(json);

            Task.Run(() => doSomethingWithToken(smartResponse));
        }

        // Use a Smart token with FHIR Net API
        public static void doSomethingWithToken(SmartResponse smartResponse)
        {
            ...
            Patient patient = fhirClient.Read<Hl7.Fhir.Model.Patient>($"Patient/{smartResponse.patientId}");

            Console.WriteLine($"Read patient: {patient.Name[0].ToString()}");
        }
